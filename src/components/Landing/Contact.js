...
import { emailValidation, contactNameValidation, contactMsgValidation } from './validations';
  ...
  return (
    <section className="flex-grow flex flex-col items-center p-10">
      <div className="flex flex-col p-5 w-full items-center bg-stone-100 opacity-90 shadow-2xl rounded-md sm:p-10 md:max-w-xl">
        ...
        <form className="w-full flex flex-col gap-5" onSubmit={handleSubmit(submitFeedback)}>
          <TextField 
            type="text" name="name" text="Name" register={register} 
            validation={contactNameValidation} errors={errors}
            hasContent={watchData.name.length > 0}
          />

          <TextField 
            type="text" name="email" text="Email" register={register} 
            validation={emailValidation} errors={errors}
            hasContent={watchData.email.length > 0}
          />

          <TextField 
            type="textarea" name="msg" text="Message" register={register} 
            validation={contactMsgValidation} errors={errors}
